...
import { DEFAULT_LANGUAGE, DEFAULT_PREFERENCES } from './config'

type GetC4SCategorySeeMoreParams = SetOptional<
...
type InferFromField<K extends string, T> = T extends { [k in K]: string }
	? T
	: never
	...
	const res = await client.GET('/clips/category/seeMore', {
		params: {
			query: {
				...
				...params,
				pill: params.pill ?? DEFAULT_PREFERENCES,
				lng: params.lng ?? DEFAULT_LANGUAGE,
				section: 'top-stores-see-more',
			},
		},
	})
	if (!res.data) throw new Error('No data')
	return res.data.seeMoreExtra as GetC4SCategorySeeMoreStudio[]
}
...
const getC4SCategorySeeMoreTopClips = async (
	...
	if (!res.data) throw new Error('No data')
	return res.data.seeMoreExtra as GetC4SCategorySeeMoreClip[]
}
